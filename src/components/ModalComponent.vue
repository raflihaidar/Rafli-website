<template>
  <div v-if="status" class="flex justify-center items-center bg-black z-50 w-screen h-screen fixed top-0 left-0">
    <transition appear name="modal" v-if="props.name === 'project'">
      <<<<<<< HEAD <div
        ...
        =======
        <div
          class="text-white bg-sky-950 p-10 transition w-[80%] h-[80%] overflow-scroll md:overflow-auto m-auto relative">
          >>>>>>> c063132df99b773871d05b156b47c9f7077f2f3c
          <section>
            ...
            <span class="w-[40%]">
              ...
              <section class="mb-3">
                <ul class="font-semibold">
                  ...
                  <li class="my-3">
                    Technologies :
                    <span class="grid lg:grid-cols-3 grid-cols-1 gap-y-3 text-center items-center text-xs mt-2 w-full">
                      <p v-for="(item, index) in data.technologies" :key="index"
    ...
    <div v-if="data.id != 1">
      <RightArrowIcon class="absolute top-1/2 md:left-20 left-3 cursor-pointer hover:scale-110 transition-transform" />
    </div>
    <div class="absolute top-1/2 md:right-20 right-3 cursor-pointer hover:scale-110 transition-transform">
      <LeftArrowIcon />
    </div>
  </div>
</template>
